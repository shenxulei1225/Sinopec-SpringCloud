...
OUTPUT = ROOT / "system/seed/dynamic_equipment_category_links.generated.sql"
TENANT_ID = 1

# legacy dynamic_model.code → EQCAT-DEV/L2 叶子或常用分类（与弱电清单品类对齐）
LEGACY_MODEL_CATEGORY: list[tuple[str, str]] = [
...
def sql_legacy_model_category_values() -> str:
    lines = [f"  ('{model}', '{cat}')" for model, cat in LEGACY_MODEL_CATEGORY]
    return ",\n".join(lines)


def build_sql() -> str:
    legacy_values = sql_legacy_model_category_values()
    return f"""-- equipment：补全 model↔category（legacy MODEL-*）+ entity↔category（按 model 主分类）
-- Regenerate: python export_equipment_category_entity_links.py
...
def main() -> None:
    ...
    print(f"wrote {OUTPUT} ({len(LEGACY_MODEL_CATEGORY)} legacy model mappings)")
