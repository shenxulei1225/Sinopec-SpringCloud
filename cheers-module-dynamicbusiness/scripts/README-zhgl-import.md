...
重启 dynamicbusiness-server 后，建议按业务类型编码调用能力重建：

```bash
# 示例：重建设备业务（businessTypeCode=equipment）
curl -X POST "http://127.0.0.1:58096/admin-api/dynamicbusiness/capability/internal/rebuild/business-type?businessTypeCode=equipment" \
