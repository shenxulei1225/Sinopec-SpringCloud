-- 统一分组模型：system_group / system_group_relation
...
CREATE INDEX IF NOT EXISTS idx_system_group_tenant_type_parent
    ON system_group(tenant_id, group_type, parent_id);
CREATE INDEX IF NOT EXISTS idx_system_group_tenant_type_name
    ON system_group(tenant_id, group_type, name);
...
CREATE TABLE IF NOT EXISTS system_group_relation (
    id BIGSERIAL PRIMARY KEY,
    tenant_id BIGINT NOT NULL DEFAULT 0,
    group_type VARCHAR(32) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_system_group_relation
    ON system_group_relation(tenant_id, group_type, group_id, target_id);
CREATE INDEX IF NOT EXISTS idx_system_group_relation_target
    ON system_group_relation(tenant_id, group_type, target_id);

-- 兼容已存在表（若之前失败时按 BIT 建过表，这里统一为 BOOLEAN）
ALTER TABLE system_group_relation
    ALTER COLUMN deleted TYPE BOOLEAN USING (CASE WHEN deleted::text IN ('1', 't', 'true') THEN TRUE ELSE FALSE END);
ALTER TABLE system_group_relation
    ALTER COLUMN deleted SET DEFAULT FALSE;

-- 迁移字段分组数据 -> 通用分组表
INSERT INTO system_group (tenant_id, group_type, code, name, description, parent_id, path, level, sort, status,
                          creator, create_time, updater, update_time, deleted)
SELECT tenant_id, 'FIELD', code, name, description, parent_id, path, level, sort, status,
       creator, create_time, updater, update_time,
       COALESCE(deleted, FALSE)
FROM system_field_group fg
WHERE NOT EXISTS (
    SELECT 1 FROM system_group g
...
INSERT INTO system_group_relation (tenant_id, group_type, group_id, target_id, sort,
                                   creator, create_time, updater, update_time, deleted)
...
FROM (
    ...
    FROM system_field_group_relation r
...
WHERE NOT EXISTS (
    SELECT 1 FROM system_group_relation gr
...
FROM (
    SELECT DISTINCT ON (r.tenant_id, r.unit_group_id, r.unit_id)
           ...
           r.update_time,
           COALESCE(r.deleted, FALSE) AS deleted
    FROM system_unit_group_relation r
