...
-- 
-- 问题说明：
-- 1. system_category表中同时存在business_type和entity_type_code两个字段
...
-- 步骤4：添加注释说明
COMMENT ON COLUMN system_category.entity_type_code IS '业务类型编码（如"region"、"equipment"）,用于区分不同的业务类型。同一业务类型下可以有多个分类,所以此字段不需要唯一性约束';
