-- ============================================================================
-- V1.0.32: 扩展 system_model_field_assignment 表支持直接存储关联目标信息
...
-- 5. 添加字段注释
COMMENT ON COLUMN system_model_field_assignment.model_relation_id 
IS '关联的 Model 关联 ID,用于级联操作';

COMMENT ON COLUMN system_model_field_assignment.target_business_type 
IS '关联目标业务类型编码（ENTITY_REF 类型字段使用）';

COMMENT ON COLUMN system_model_field_assignment.target_model_code 
IS '关联目标 Model 编码（ENTITY_REF 类型字段使用）';

COMMENT ON COLUMN system_model_field_assignment.display_field_code 
...
CREATE INDEX IF NOT EXISTS idx_model_field_assignment_relation 
    ON system_model_field_assignment(model_relation_id) 
...
CREATE INDEX IF NOT EXISTS idx_model_field_assignment_target 
    ON system_model_field_assignment(target_business_type, target_model_code) 
