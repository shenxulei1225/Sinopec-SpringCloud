-- ============================================================================
-- V1.0.37: 将 is_searchable 和 is_sortable 从字段级别移到模型字段分配级别
...
-- 3. 添加字段注释
COMMENT ON COLUMN system_model_field_assignment.is_searchable 
IS '是否可查询（如果为 NULL,则使用字段定义中的默认值）';

COMMENT ON COLUMN system_model_field_assignment.is_sortable 
IS '是否可排序（如果为 NULL,则使用字段定义中的默认值）';

-- 4. 数据迁移：将现有字段定义中的 is_searchable 和 is_sortable 值迁移到模型字段分配中
-- 注意：只迁移已存在的字段分配记录,新创建的字段分配将使用 NULL（表示使用字段定义中的默认值）
UPDATE system_model_field_assignment mfa
SET 
...
-- 5. 从 system_field 表中移除 is_searchable 和 is_sortable 字段
-- 注意：这些字段现在在模型字段分配级别配置,不再在字段定义级别配置
