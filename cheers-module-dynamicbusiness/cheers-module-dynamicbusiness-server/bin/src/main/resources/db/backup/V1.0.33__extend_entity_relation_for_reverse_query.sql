...
COMMENT ON COLUMN system_entity_relation.source_business_type_code IS '源业务类型编码,用于跨业务类型查询';
COMMENT ON COLUMN system_entity_relation.target_business_type_code IS '目标业务类型编码,用于跨业务类型查询';
...
ON system_entity_relation(target_business_type_code, target_entity_id) 
WHERE deleted = FALSE;

-- 索引5：按源业务类型查询（跨业务类型统计）
CREATE INDEX IF NOT EXISTS idx_entity_relation_source_business_type 
ON system_entity_relation(source_business_type_code, source_entity_id) 
WHERE deleted = FALSE;
