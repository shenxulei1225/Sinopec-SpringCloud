...
-- 页面配置新增 page_code 字段
-- 版本: V1.0.65
-- 描述: 为 system_page_config 增加 page_code，用于页面唯一定位
-- =====================================================

ALTER TABLE system_page_config
ADD COLUMN IF NOT EXISTS page_code VARCHAR(100);

COMMENT ON COLUMN system_page_config.page_code IS '页面代码（唯一标识）';
...
-- 创建唯一索引
CREATE UNIQUE INDEX IF NOT EXISTS idx_page_code ON system_page_config(page_code);
