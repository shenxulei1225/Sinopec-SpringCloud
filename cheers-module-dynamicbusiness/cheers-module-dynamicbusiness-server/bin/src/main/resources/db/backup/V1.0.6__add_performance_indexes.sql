...
--     ON system_entity USING gin (to_tsvector('simple', custom_fields));

-- =====================================================
-- 4. 实体分类关联表 (system_entity_category_relation) 索引优化
-- =====================================================

-- 实体ID索引（按实体查询分类）
CREATE INDEX IF NOT EXISTS idx_entity_category_entity_id 
    ON system_entity_category_relation(entity_id);

-- 分类ID索引（按分类查询实体）
CREATE INDEX IF NOT EXISTS idx_entity_category_category_id 
    ON system_entity_category_relation(category_id);

-- 复合索引：实体ID + 分类ID（关联查询优化）
CREATE INDEX IF NOT EXISTS idx_entity_category_entity_category 
    ON system_entity_category_relation(entity_id, category_id);

-- 删除标记索引
CREATE INDEX IF NOT EXISTS idx_entity_category_deleted 
    ON system_entity_category_relation(deleted);

-- =====================================================
-- 5. 模型表 (system_model) 索引优化
-- =====================================================

-- 模型编码索引
CREATE INDEX IF NOT EXISTS idx_model_code 
    ON system_model(code);

-- 业务类型编码索引
CREATE INDEX IF NOT EXISTS idx_model_business_type_code 
    ON system_model(business_type_code);

-- 状态索引
CREATE INDEX IF NOT EXISTS idx_model_status 
    ON system_model(status);

-- 删除标记索引
CREATE INDEX IF NOT EXISTS idx_model_deleted 
    ON system_model(deleted);

-- 复合索引：业务类型 + 状态 + 删除标记
CREATE INDEX IF NOT EXISTS idx_model_business_status_deleted 
    ON system_model(business_type_code, status, deleted);

-- =====================================================
-- 6. 模型字段分配表 (system_model_field_assignment) 索引优化
-- =====================================================

-- 模型ID索引
CREATE INDEX IF NOT EXISTS idx_model_field_model_id 
    ON system_model_field_assignment(model_id);

-- 字段ID索引
CREATE INDEX IF NOT EXISTS idx_model_field_field_id 
    ON system_model_field_assignment(field_id);

-- 复合索引：模型ID + 字段ID
CREATE INDEX IF NOT EXISTS idx_model_field_model_field 
    ON system_model_field_assignment(model_id, field_id);

-- 删除标记索引
CREATE INDEX IF NOT EXISTS idx_model_field_deleted 
    ON system_model_field_assignment(deleted);

-- 7. 单位表 (system_unit) 索引优化（如果存在）
--    注意：当前环境可能尚未创建 system_unit 表,这里增加存在性判断,避免 Flyway 执行失败
-- =====================================================

DO $$
BEGIN
    -- 仅当当前 schema 下存在 system_unit 表时才创建索引
    IF EXISTS (
        SELECT 1
        FROM information_schema.tables
        WHERE table_schema = current_schema()
          AND table_name = 'system_unit'
    ) THEN
        -- 单位编码索引
        ...
        CREATE INDEX IF NOT EXISTS idx_unit_status
            ON system_unit(status);

        -- 删除标记索引
        CREATE INDEX IF NOT EXISTS idx_unit_deleted
            ON system_unit(deleted);
    END IF;
END
$$;
