...
-- 添加字段注释
COMMENT ON COLUMN system_business_type.association_fields IS '可用的关联字段定义（JSONB格式）。存储该业务类型可供其他业务类型关联的字段定义。格式：{"字段key": {"fieldType": "REF_Multi", "refEntityTypeCode": "xxx", ...}}。Model创建时可以从这些字段中选择使用。';
...
--   SELECT * FROM system_business_type WHERE association_fields @> '{"equipment_ref": {"refEntityTypeCode": "equipment"}}';
