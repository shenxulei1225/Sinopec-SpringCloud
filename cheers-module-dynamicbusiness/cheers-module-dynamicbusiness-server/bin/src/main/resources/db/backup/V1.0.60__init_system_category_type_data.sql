-- =====================================================
-- 初始化 system_category_type 数据
-- 描述：将 system_category 表中出现过的 category_type_code（去重）同步到 system_category_type,
--       每个 code 只插入一条记录,name 使用 category_type_code,后续可在管理端修改。
--       表唯一索引为 category_type_code,故按 code 维度补全即可。
-- =====================================================

INSERT INTO system_category_type (category_type_code, name, description, status, top_level_category_id, creator, create_time, updater, update_time, deleted, tenant_id)
SELECT
