-- =====================================================
-- 添加 index_strategy 字段到 system_field 表
-- 需求：FR-BDA-080~083 智能默认可查询
-- =====================================================

-- 添加 index_strategy 字段
ALTER TABLE system_field ADD COLUMN IF NOT EXISTS index_strategy VARCHAR(32) DEFAULT 'NONE';

-- 添加字段注释
COMMENT ON COLUMN system_field.index_strategy IS '索引策略：GIN-GIN索引（等值/包含查询）,BTREE-B-Tree索引（范围查询/排序）,NONE-不索引';
