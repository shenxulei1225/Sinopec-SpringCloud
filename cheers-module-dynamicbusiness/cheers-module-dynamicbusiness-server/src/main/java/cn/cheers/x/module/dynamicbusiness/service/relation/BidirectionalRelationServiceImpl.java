...
public class BidirectionalRelationServiceImpl implements BidirectionalRelationService {
    ...
    public List<RelatedEntityVO> getForwardRelations(Long entityId) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        // 尝试从通用表查询，如果失败则返回空列表
        log.warn("[getForwardRelations][使用了已废弃的方法，entityId={}，建议使用 getForwardRelations(entityId, businessTypeCode)]", entityId);
        try {
            // 尝试使用默认的 businessTypeCode（通用表）
            ...
            log.warn("[getForwardRelations][查询失败，entityId={}，可能是动态表实体，请使用带 businessTypeCode 的方法]", entityId);
            return Collections.emptyList();
        }
    }

    @Override
    public List<RelatedEntityVO> getForwardRelations(Long entityId, String businessTypeCode) {
        EntityDO entityDO = entityCoreService.get(entityId, businessTypeCode);
        EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
        if (entity == null) return Collections.emptyList();
        ModelDO model = modelMapper.selectById(entity.getModelId());
        if (model == null) return Collections.emptyList();
    ...
    private List<RelatedEntityVO> getForwardRelationsInternal(Long entityId, EntityRespVO entity, ModelDO model) {
        ...
        for (ModelFieldAssignmentDO assignment : assignments) {
            ...
            String refBusinessTypeCode = resolveRefBusinessTypeCode(field, refEntityId, assignment);
            if (refBusinessTypeCode == null) {
                log.warn("[getForwardRelationsInternal][无法确定引用实体的 businessTypeCode，fieldCode={}, refEntityId={}]", field.getCode(), refEntityId);
                continue;
            }
            ...
            result.add(RelatedEntityVO.builder().modelCode(refModel.getCode()).modelName(refModel.getName())
                    ...
                    .businessTypeCode(refModel.getBusinessTypeCode()).autoGenerated(false)
    ...
    public List<RelatedEntityVO> getReverseRelations(Long entityId) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        log.warn("[getReverseRelations][使用了已废弃的方法，entityId={}，建议使用 getReverseRelations(entityId, businessTypeCode)]", entityId);
        try {
            // 尝试使用默认的 businessTypeCode（通用表）
            EntityDO entityDO = entityCoreService.get(entityId, null);
            EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
            ...
            log.warn("[getReverseRelations][查询失败，entityId={}，可能是动态表实体，请使用带 businessTypeCode 的方法]", entityId);
            return Collections.emptyList();
        }
    }

    @Override
    public List<RelatedEntityVO> getReverseRelations(Long entityId, String businessTypeCode) {
        EntityDO entityDO = entityCoreService.get(entityId, businessTypeCode);
        EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
        if (entity == null) return Collections.emptyList();
        ModelDO model = modelMapper.selectById(entity.getModelId());
    ...
    private List<RelatedEntityVO> getReverseRelationsInternal(Long entityId, ModelDO model) {
        ...
        if (CollUtil.isNotEmpty(relations)) {
            ...
            for (Map.Entry<String, List<EntityRelationDO>> entry : groupedRelations.entrySet()) {
                ...
                String businessTypeCode = firstRelation.getSourceBusinessTypeCode();
                if (businessTypeCode == null && sourceModel != null) {
                    businessTypeCode = sourceModel.getBusinessTypeCode();
                }
        ...
        for (RelationDiscoveryVO discovery : discoveries) {
            ...
            result.add(RelatedEntityVO.builder().modelCode(discovery.getSourceModelCode())
                    ...
                    .businessTypeCode(discovery.getSourceBusinessTypeCode())
                    .businessTypeName(discovery.getSourceBusinessTypeName())
    ...
    public PageResult<EntitySimpleVO> getReverseRelationsByModel(Long entityId, String modelCode, PageParam pageParam) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        log.warn("[getReverseRelationsByModel][使用了已废弃的方法，entityId={}，建议使用 getReverseRelationsByModel(entityId, businessTypeCode, modelCode, pageParam)]", entityId);
        try {
            // 尝试使用默认的 businessTypeCode（通用表）
            EntityDO entityDO = entityCoreService.get(entityId, null);
            EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
            ...
            log.warn("[getReverseRelationsByModel][查询失败，entityId={}，可能是动态表实体，请使用带 businessTypeCode 的方法]", entityId);
            return new PageResult<>(Collections.emptyList(), 0L);
        }
    }

    @Override
    public PageResult<EntitySimpleVO> getReverseRelationsByModel(Long entityId, String businessTypeCode, String modelCode, PageParam pageParam) {
        EntityDO entityDO = entityCoreService.get(entityId, businessTypeCode);
        EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
        if (entity == null) return new PageResult<>(Collections.emptyList(), 0L);
        ModelDO targetModel = modelMapper.selectById(entity.getModelId());
    ...
    private PageResult<EntitySimpleVO> getReverseRelationsByModelInternal(Long entityId, ModelDO targetModel, String modelCode, PageParam pageParam) {
        ...
        // 获取 sourceModel 的 businessTypeCode
        String sourceBusinessTypeCode = sourceModel.getBusinessTypeCode();
        if (sourceBusinessTypeCode == null) {
            log.warn("[getReverseRelationsByModelInternal][sourceModel 没有配置 businessTypeCode，modelCode={}]", sourceModel.getCode());
        }
        ...
        if (CollUtil.isNotEmpty(relations)) {
            ...
            for (Long eid : entityIds) {
                try {
                    EntityDO eDO = entityCoreService.get(eid, sourceBusinessTypeCode);
                    if (eDO != null) {
                        EntityRespVO e = EntityDoVoHelper.toRespVO(eDO, customFieldValidationService);
                        entities.add(buildEntitySimpleVO(e, sourceModel));
                    }
                } catch (Exception ex) {
                    log.warn("[getReverseRelationsByModelInternal][查询实体失败，entityId={}, businessTypeCode={}]", eid, sourceBusinessTypeCode);
                }
            }
    ...
    public RelationStatisticsVO getRelationStatistics(Long entityId) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        log.warn("[getRelationStatistics][使用了已废弃的方法，entityId={}，建议使用 getRelationStatistics(entityId, businessTypeCode)]", entityId);
        try {
            // 尝试使用默认的 businessTypeCode（通用表）
            EntityDO entityDO = entityCoreService.get(entityId, null);
            EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
            ...
            log.warn("[getRelationStatistics][查询失败，entityId={}，可能是动态表实体，请使用带 businessTypeCode 的方法]", entityId);
            return buildEmptyStatistics(entityId);
        }
    }

    @Override
    public RelationStatisticsVO getRelationStatistics(Long entityId, String businessTypeCode) {
        EntityDO entityDO = entityCoreService.get(entityId, businessTypeCode);
        EntityRespVO entity = entityDO != null ? EntityDoVoHelper.toRespVO(entityDO, customFieldValidationService) : null;
        if (entity == null) return buildEmptyStatistics(entityId);
        return getRelationStatisticsInternal(entityId, entity);
    ...
    private RelationStatisticsVO getRelationStatisticsInternal(Long entityId, EntityRespVO entity) {
        ...
        // 获取 Model 以确定 businessTypeCode
        ModelDO model = modelMapper.selectById(entity.getModelId());
        String businessTypeCode = model != null ? model.getBusinessTypeCode() : null;
        
        // 获取正向关联统计
        List<RelatedEntityVO> forwardRelations = businessTypeCode != null 
            ? getForwardRelations(entityId, businessTypeCode) 
        ...
        if (CollUtil.isNotEmpty(groupedStats)) {
            ...
            List<RelatedEntityVO> reverseRelations = businessTypeCode != null 
                ? getReverseRelations(entityId, businessTypeCode) 
    ...
    public List<AggregateResultVO> aggregateByRelation(AggregateQueryReqVO reqVO) {
        ...
        for (Map.Entry<Long, List<EntityFieldIndexDO>> entry : groupedRecords.entrySet()) {
            ...
            String refBusinessTypeCode = resolveRefBusinessTypeCode(groupByField, groupKey, groupAssignment);
            EntityRespVO relatedEntity = null;
            if (refBusinessTypeCode != null) {
                try {
                    relatedEntity = entityService.get(groupKey, refBusinessTypeCode);
                } catch (Exception ex) {
                    log.warn("[aggregateByRelation][查询引用实体失败，groupKey={}, businessTypeCode={}]", groupKey, refBusinessTypeCode);
                }
            } else {
                log.warn("[aggregateByRelation][无法确定引用实体的 businessTypeCode，groupKey={}]", groupKey);
            }
        ...
        for (ModelDO sourceModel : allModels) {
            ...
            for (ModelFieldAssignmentDO assignment : assignments) {
                ...
                result.add(RelationDiscoveryVO.builder().sourceModelId(sourceModel.getId()).sourceModelCode(sourceModel.getCode())
                        .sourceModelName(sourceModel.getName()).sourceBusinessTypeCode(sourceModel.getBusinessTypeCode())
    ...
    public EntityRelationInfoVO getEntityRelationInfo(Long entityId) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        log.warn("[getEntityRelationInfo][使用了已废弃的方法，entityId={}，建议使用 getEntityRelationInfo(entityId, businessTypeCode)]", entityId);
        try {
            // 尝试使用默认的 businessTypeCode（通用表）
            ...
            log.warn("[getEntityRelationInfo][查询失败，entityId={}，可能是动态表实体，请使用带 businessTypeCode 的方法]", entityId);
            return null;
        }
    }

    @Override
    public EntityRelationInfoVO getEntityRelationInfo(Long entityId, String businessTypeCode) {
    ...
    private EntityRelationInfoVO getEntityRelationInfoInternal(Long entityId, EntityRespVO entity, ModelDO model) {
        ...
        return EntityRelationInfoVO.builder().entityId(entityId).entityName(entity.getName()).modelCode(model.getCode())
                ...
                .forwardRelations(businessTypeCode != null ? getForwardRelations(entityId, businessTypeCode) : getForwardRelations(entityId))
                .reverseRelations(businessTypeCode != null ? getReverseRelations(entityId, businessTypeCode) : getReverseRelations(entityId))
                .statistics(businessTypeCode != null ? getRelationStatistics(entityId, businessTypeCode) : getRelationStatistics(entityId)).build();
    }
    ...
    public Map<Long, Long> batchGetReverseRelationCounts(List<Long> entityIds) {
        // 已废弃：无法确定 businessTypeCode，无法正确路由到动态表
        log.warn("[batchGetReverseRelationCounts][使用了已废弃的方法，建议使用 batchGetReverseRelationCounts(businessTypeCode, entityIds)]");
    ...
    @Override
    public Map<Long, Long> batchGetReverseRelationCounts(String businessTypeCode, List<Long> entityIds) {
        if (CollUtil.isEmpty(entityIds)) return Collections.emptyMap();
        Map<Long, Long> result = new HashMap<>();
        for (Long entityId : entityIds) {
            List<RelatedEntityVO> reverseRelations = getReverseRelations(entityId, businessTypeCode);
            result.put(entityId, reverseRelations.stream().mapToLong(RelatedEntityVO::getCount).sum());
        }
        return result;
    }
    ...
    /**
     * 根据模型字段分配解析引用实体所在 businessTypeCode。
     */
    private String resolveRefBusinessTypeCode(FieldDO field, Long refEntityId, ModelFieldAssignmentDO assignment) {
        if (assignment == null) {
            return null;
        }
        if (assignment.getRefLibraryId() != null) {
            ...
            if (lib != null && StrUtil.isNotBlank(lib.getRefBusinessType())) {
        ...
        if (StrUtil.isNotBlank(assignment.getTargetBusinessType())) {
    ...
    private boolean isAssignmentReferencingTargetModel(ModelFieldAssignmentDO assignment, ModelDO targetModel) {
        ...
        if (assignment.getRefLibraryId() != null) {
            RelationFieldLibraryDO lib = relationFieldLibraryMapper.selectById(assignment.getRefLibraryId());
            return lib != null && StrUtil.isNotBlank(lib.getRefBusinessType())
                    && lib.getRefBusinessType().equals(targetModel.getBusinessTypeCode());
        }
        return StrUtil.isNotBlank(assignment.getTargetBusinessType())
                && assignment.getTargetBusinessType().equals(targetModel.getBusinessTypeCode());
    }
