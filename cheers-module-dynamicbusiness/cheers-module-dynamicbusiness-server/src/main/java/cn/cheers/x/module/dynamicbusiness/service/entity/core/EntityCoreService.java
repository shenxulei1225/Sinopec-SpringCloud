...
public interface EntityCoreService {
     ...
     *
     * @param entityId 实体ID
     * @param entityTypeCode 业务类型编码（用于路由动态表）
     * @return true=存在；false=不存在
     */
    boolean existsById(Long entityId, String entityTypeCode);
     ...
     * @return 存在的实体ID集合
     */
    Set<Long> filterExistingEntityIds(List<Long> entityIds, String entityTypeCode);
     ...
     */
    EntityDO get(Long id, String entityTypeCode);
     ...
     * @return 实体DO列表
     */
    List<EntityDO> listEntities(String entityTypeCode, Long modelId, Integer status);
     ...
     * @return 分页结果
     */
    PageResult<EntityDO> pageEntities(String entityTypeCode, Long modelId, Integer status, String keyword, Integer pageNo, Integer pageSize);
     ...
     * @return 分页结果
     */
    PageResult<EntityDO> pageEntitiesByModelIds(String entityTypeCode, List<Long> modelIds, Integer status,
     ...
     * @return 实体DO扁平列表
     */
    List<EntityDO> listTreeEntities(String entityTypeCode, Long modelId);

    /**
     * 按父节点查询直接子实体；parentId 为 null 时仅返回根节点（parent_id IS NULL）。
     */
    List<EntityDO> listEntitiesByParentId(String entityTypeCode, Long parentId);

    /**
     * 按父节点分页查询直接子实体；parentId 为 null 时仅返回根节点（parent_id IS NULL）。
     */
    PageResult<EntityDO> pageEntitiesByParentId(String entityTypeCode, Long parentId, Integer pageNo, Integer pageSize);
     ...
     * @return 实体ID列表
     */
    List<Long> getEntityIdsByModelId(Long modelId, String entityTypeCode);
