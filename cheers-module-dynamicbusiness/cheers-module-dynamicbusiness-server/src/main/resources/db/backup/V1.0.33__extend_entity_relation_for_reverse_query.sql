-- =====================================================
-- 扩展 dynamic_entity_relation 表,支持高效的反向查询和统计
...
ON dynamic_entity_relation(target_entity_id, source_model_code) 
WHERE deleted = FALSE;

-- 索引2：按目标实体和字段编码查询（按字段分组统计）
CREATE INDEX IF NOT EXISTS idx_entity_relation_target_field 
ON dynamic_entity_relation(target_entity_id, field_code) 
WHERE deleted = FALSE;

-- 索引3：按源实体和字段编码查询（正向查询优化）
CREATE INDEX IF NOT EXISTS idx_entity_relation_source_field 
ON dynamic_entity_relation(source_entity_id, field_code) 
WHERE deleted = FALSE;
...
ON dynamic_entity_relation(target_entity_id, source_model_code, field_code) 
WHERE deleted = FALSE;
