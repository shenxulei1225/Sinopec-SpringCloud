...
-- 
-- 变更内容：
-- 1. 移除 dynamic_model 表的 parent_model_id 和 is_template 字段
-- 2. 移除相关索引
-- 
-- 注意：
...
ALTER TABLE dynamic_model DROP COLUMN IF EXISTS parent_model_id;

-- 3. 移除 is_template 字段
ALTER TABLE dynamic_model DROP COLUMN IF EXISTS is_template;

-- 4. 添加注释说明变更
COMMENT ON TABLE dynamic_model IS '业务模型表（Template-Model 复制机制,移除继承字段）';
