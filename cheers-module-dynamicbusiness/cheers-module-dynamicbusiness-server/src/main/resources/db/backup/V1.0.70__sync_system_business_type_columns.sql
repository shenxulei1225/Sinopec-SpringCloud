-- ====================================================================
-- V1.0.68: 同步 dynamic_business_type 表缺失的列
...
COMMENT ON COLUMN dynamic_business_type.parent_id IS '父业务类型 ID,支持业务类型树结构';
...
COMMENT ON COLUMN dynamic_business_type.association_fields IS '可用的关联字段定义（JSONB 格式）。';
...
COMMENT ON COLUMN dynamic_business_type.storage_type IS '存储类型：GENERIC/DEDICATED_STATIC/DEDICATED_DYNAMIC';
COMMENT ON COLUMN dynamic_business_type.dedicated_table_name IS '专用表名';
COMMENT ON COLUMN dynamic_business_type.enable_rule_engine IS '是否启用规则引擎';
COMMENT ON COLUMN dynamic_business_type.physical_column_mapping IS '物理列映射配置（JSON 格式）。';
