-- =====================================================
-- 为 dynamic_entity_category_relation 补充 business_type_code 相关索引
-- 日期：2026-03-20
-- 说明：
...
COMMENT ON COLUMN dynamic_entity_category_relation.business_type_code IS '实体业务类型编码（如 region、equipment）';
...
CREATE INDEX IF NOT EXISTS idx_ecr_category_business_tenant_not_deleted
    ON dynamic_entity_category_relation (category_id, business_type_code, tenant_id)
    WHERE deleted = FALSE;

-- 2. 场景：按 entityId + businessTypeCode 反查分类/做关联处理
CREATE INDEX IF NOT EXISTS idx_ecr_entity_business_tenant_not_deleted
    ON dynamic_entity_category_relation (entity_id, business_type_code, tenant_id)
    WHERE deleted = FALSE;
