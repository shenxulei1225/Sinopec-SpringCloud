...
CREATE UNIQUE INDEX IF NOT EXISTS uk_dynamic_reference_provider_code
    ON dynamic_reference_provider (provider_code, tenant_id, deleted);
...
COMMENT ON COLUMN dynamic_field.provider_code IS '引用 Provider 编码（REFERENCE/ENTITY_REF 可选）';
COMMENT ON COLUMN dynamic_field.semantic_type IS '引用语义类型（USER/DEPT/ROLE/MATERIAL...）';

-- 初始化默认 dynamic_USER provider
INSERT INTO dynamic_reference_provider
...
SELECT 'dynamic_USER', '系统用户', 'INTERNAL', 'USER', '{"supports_search":true,"supports_batch_validate":false}', 1, 'GLOBAL', 100, 'UNKNOWN', 0
WHERE NOT EXISTS (
    SELECT 1 FROM dynamic_reference_provider
    WHERE provider_code = 'dynamic_USER' AND tenant_id = 0 AND deleted = B'0'
);
