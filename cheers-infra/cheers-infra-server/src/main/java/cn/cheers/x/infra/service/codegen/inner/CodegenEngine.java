package cn.cheers.x.infra.service.codegen.inner;
...
import cn.cheers.x.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.cheers.x.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.cheers.x.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/cheers-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/cheers-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "cheers-${table.moduleName}/" + // 顶级模块
                "cheers-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "cheers-${table.moduleName}/" + // 顶级模块
                "cheers-${table.moduleName}-server/" + // 子模块
    ...
    private static String vue3UniappFilePath(String path) {
        return "cheers-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "cheers-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
