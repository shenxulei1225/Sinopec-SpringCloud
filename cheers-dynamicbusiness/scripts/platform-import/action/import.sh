...
echo "== action: task head/tail actions =="
run "${SCRIPT_DIR}/13_head_tail_actions.sql"
echo "== action: retire param models (keep hidden execution_means) =="
run "${SCRIPT_DIR}/12_retire_param_models_and_execution_means.sql"
echo "== action: restore hidden execution_means =="
run "${SCRIPT_DIR}/14_restore_execution_means_hidden.sql"
echo "== action: execution means enum field =="
run "${SCRIPT_DIR}/15_execution_means_category.sql"
echo "== action: action duration param =="
run "${SCRIPT_DIR}/16_action_duration.sql"
