...
def apply(cur, dry_run: bool = False, *, prune: bool = False, entity_type_codes: list[str] | None = None) -> tuple[int, int, int]:
    upserted = removed = 0
    canonical_codes: set[tuple[str, str]] = set()
    scope = entity_type_codes if entity_type_codes else list(CANONICAL_BASE_FIELDS.keys())

    for entity_type_code in scope:
        fields = CANONICAL_BASE_FIELDS.get(entity_type_code, [])
        for spec in fields:
            canonical_codes.add((entity_type_code, spec["field_code"]))
            type_config = spec.get("type_config")
            tc_json = json.dumps(type_config, ensure_ascii=False) if type_config else None
            if dry_run:
                print(f"  upsert {entity_type_code}.{spec['field_code']}")
                upserted += 1
                continue
            cur.execute(
                """
                INSERT INTO dynamic_entity_type_base_field (
                  entity_type_code, field_code, field_name, data_type, required,
    ...
    cur.execute(
        """
        SELECT id, entity_type_code, field_code
    ...
    if not prune:
        if not dry_run:
            cur.execute(
                """
                SELECT COUNT(*) FROM dynamic_entity_type_base_field
                WHERE deleted = false AND tenant_id = %s
                """,
                (TENANT_ID,),
            )
...
def main():
    ...
    parser.add_argument(
        "--entity-type",
        action="append",
        dest="entity_type_codes",
        metavar="CODE",
        help="Apply/prune only these entity type codes (repeatable). Default: all in canonical.",
    )
    parser.add_argument(
        "--business-type",
        action="append",
        dest="entity_type_codes",
        metavar="CODE",
        help="Deprecated alias of --entity-type.",
    )
    ...
    try:
        ...
        scope = args.entity_type_codes or list(CANONICAL_BASE_FIELDS.keys())
        print(f"Applying canonical base fields for: {', '.join(scope)}...")
        upserted, removed, active = apply(
            cur,
            dry_run=args.dry_run,
            prune=args.prune,
            entity_type_codes=args.entity_type_codes,
        )
        if args.dry_run:
