...
/**
 * 五维编排场景配方 id（平台内置；写入库后运行时只读该目录自己的行）。
 */
@Getter
@AllArgsConstructor
public enum FiveWRecipeIdEnum {
    ...
    public static FiveWRecipeIdEnum fromRecipeId(String raw) {
        if (raw == null || raw.isBlank()) {
            return null;
        }
        String normalized = raw.trim();
        for (FiveWRecipeIdEnum value : values()) {
