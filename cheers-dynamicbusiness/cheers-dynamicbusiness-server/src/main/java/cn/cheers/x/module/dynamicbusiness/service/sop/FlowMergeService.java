...
/**
 * 标准流程读时 merge：全平台唯一合并入口（后端）。
 * 与前端 {@code mergeFlowEffectiveConfig} 契约对齐；不解析业务巡检点 / 停靠站。
 */
public interface FlowMergeService {

    /**
     * 合并标准流程与差量，并校验参数槽是否已填值（任务生效用）。
     *
     * @param standard      标准流程快照（actionTree / paramsByNode）
     * @param treeOverride  动作树差量；null 或空 replaceTree 则用标准树
     * @param paramOverride 按 nodeKey 的参数差量
     */
    default FlowMergeResult merge(FlowStandardSnapshot standard,
     ...
     */
    FlowMergeResult merge(FlowStandardSnapshot standard,
                         FlowTreeOverride treeOverride,
                         Map<String, Map<String, Object>> paramOverride,
