-- V116: 修复 inspection_item 步骤字段权威路径（V115 已执行后的增量纠偏）
-- 背景：
-- - V115 在部分环境已按旧内容执行并写入历史；随后做过 flyway repair。
-- - 本迁移不回滚历史版本，只做前向收敛：统一回「基础字段 + 物理列 + BASE 分配」。
--
-- 目标：
-- 1) 补 ent_inspection_item / ent_inspection_item_t* 的 step_tree_json 物理列；
-- 2) 确保 inspection_item 的 step_tree_json 基础字段为有效态；
-- 3) 检查项模型分配 step_tree_json 统一为 BASE；
-- 4) 清理 action_tree_json 历史分配，避免双键并行；
-- 5) 清缓存，触发 CRUD 投影重算。
...
BEGIN
  ...
  LOOP
    ...
    -- 兼容历史错误路径：若 custom_fields 里已写过 step_tree_json/action_tree_json，则迁回物理列。
    EXECUTE format(
      $sql$
      ...
      SET step_tree_json = CASE
            ...
            ELSE step_tree_json
          END,
          updater = COALESCE(updater, 'flyway-v116'),
...
-- 3) 先关闭 inspection_item 上已删除的历史基础字段残留（同编码旧行）
UPDATE dynamic_entity_type_base_field
SET update_time = CURRENT_TIMESTAMP,
    updater = 'flyway-v116'
WHERE entity_type_code = 'inspection_item'
  AND field_code = 'step_tree_json'
  AND deleted = TRUE;

-- 4) 确保 inspection_item 基础字段有效行存在
...
-- 6) 退役 inspection_item 历史 action_tree_json 分配
