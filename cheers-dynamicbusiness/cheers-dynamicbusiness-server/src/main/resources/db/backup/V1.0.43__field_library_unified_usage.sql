...
BEGIN
    ...
    LOOP
        ...
        SELECT jsonb_object_agg(
            COALESCE(
                (SELECT sf.code FROM dynamic_field sf WHERE sf.id = key::bigint AND sf.deleted = FALSE),
                key
            ),
            value
...
BEGIN
    SELECT COUNT(*) INTO total_entities FROM dynamic_entity WHERE deleted = FALSE;
...
-- CREATE INDEX IF NOT EXISTS idx_entity_custom_fields_field_codes ON dynamic_entity USING GIN (custom_fields);

-- =====================================================
-- 7. 迁移完成标记
-- =====================================================
INSERT INTO dynamic_data_migration_log (
    migration_type,
    migration_version,
    status,
    start_time,
    ...
    CURRENT_TIMESTAMP,
    CURRENT_TIMESTAMP,
    (SELECT COUNT(*) FROM dynamic_entity WHERE deleted = FALSE AND custom_fields IS NOT NULL AND custom_fields != '{}'),
    (SELECT COUNT(*) FROM dynamic_entity WHERE deleted = FALSE AND custom_fields::text LIKE '%"F-%":%'),
    (SELECT jsonb_build_object(
        'description', '字段库统一使用规范迁移：fieldId到fieldCode',
        'migration_type', 'data_format_migration',
        'affected_table', 'dynamic_entity',
        'affected_column', 'custom_fields',
        'fully_migrated_count', (SELECT COUNT(*) FROM dynamic_entity WHERE deleted = FALSE AND custom_fields IS NOT NULL AND custom_fields != '{}' AND custom_fields::text LIKE '%"F-%":%' AND custom_fields::text NOT LIKE '%\d+":%'),
        'partially_migrated_count', (SELECT COUNT(*) FROM dynamic_entity WHERE deleted = FALSE AND custom_fields IS NOT NULL AND custom_fields != '{}' AND custom_fields::text LIKE '%"F-%":%' AND custom_fields::text LIKE '%\d+":%'),
        'unmigrated_count', (SELECT COUNT(*) FROM dynamic_entity WHERE deleted = FALSE AND custom_fields IS NOT NULL AND custom_fields != '{}' AND custom_fields::text NOT LIKE '%"F-%":%' AND custom_fields::text LIKE '%\d+":%')
    )),
    'system'
);